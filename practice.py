...
class Cities:
    ...
    def inc_vote(self,frame):
        self.vote += 1
        print("dramu")
        #tried printing self.vote
        frame.pack_forget()
        butto_1 = Button(root, text = "Ramu is literally god").pack()
...
class Questions:
    ...
    def get_option(self,root):
        ...
        for i in range(0,len(self.options)):
            button = Button(frame, text = self.options[i].natural, command = lambda *args: self.options[i].inc_vote(frame)).pack()
        return frame
